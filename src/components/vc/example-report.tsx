...
export default function ExampleReport() {
  ...
  return (
    <motion.div
      ...
      variants={subeSuave}
      className="border-[3px] border-vc-marron bg-vc-amarillo p-7 shadow-dura-marron md:p-10"
    >
      <div className="flex flex-wrap items-start justify-between gap-x-6 gap-y-3 border-b-2 border-vc-marron/25 pb-5">
        <h2 className="max-w-2xl font-display text-3xl font-black uppercase leading-none text-vc-marron md:text-[2.75rem]">
          {t('home.caseTitle')}
        </h2>
        <Etiqueta tono="oscuro">{t('home.caseBadge')}</Etiqueta>
      </div>
      ...
      <dl className="mt-8 grid gap-x-8 gap-y-6 sm:grid-cols-3">
        {METRICAS.map((m) => (
          <div key={m.labelKey} className="border-t-[3px] border-vc-marron pt-4">
            ...
            <p className="mt-1 font-mono text-[11px] font-semibold uppercase tracking-wider text-vc-marron/70">
      ...
      <p className="mt-8 border-t-2 border-vc-marron/25 pt-5 text-sm leading-relaxed text-vc-marron/80">
        {t('home.caseQuote')}
      </p>
    </motion.div>
  );
